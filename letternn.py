...
with open('AB Dataset.csv', 'r') as file:
    ...
    # images = [(
    #     np.array(image[1:]).astype(int), np.asarray(int(image[0]))
    #     ) for image in line_reader]

    images = [(
        np.array(image[1:]).astype(int),
        np.asarray([1, 0]) if int(image[0]) == 1 else np.asarray([0, 1])
    ) for image in line_reader]
    # images = [[
    #     np.array(image[1:]).astype(int).reshape((28,28)), letters[int(image[0])]
    #     ] for image in line_reader]
    file.close()
